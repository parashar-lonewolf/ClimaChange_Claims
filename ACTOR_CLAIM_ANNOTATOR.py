...
def _main():
	...
	saved_anno = False
	tot_articles = int(len(textlist)/2)
	if(os.path.exists(SAVE_DATA)):
		n_cnt = restart_index
		saved_anno = True
	...
	for i in textlist[2*restart_index:]:
		if(n_cnt!=0):
			n_cnt,found = repeat_annotator(i,nlp,saved_anno,n_cnt,tot_articles,n_line,progress_line,found,SAVE_DATA,OUTPUT_FILE,ConLL_FILE)
	return 0
...
def SAVE_DATA_fun(save_data,SAVE_DATA):
